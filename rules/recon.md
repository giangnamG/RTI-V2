...
- **Trigger**: có web port (`service_category='web'`). **Prerequisite**: SCAN_PORT.
- **Làm gì**: build URL `scheme://host:port` (port luôn tường minh) → httpx (`-title -status-code -tech-detect -server -follow-redirects -max-redirects 3 -timeout 10 -threads 50`) → merge tech từ whatweb (detect CMS). → bảng `web_probes`.
