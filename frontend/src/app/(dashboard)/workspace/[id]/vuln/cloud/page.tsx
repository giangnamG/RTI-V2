...
const TOOLS = ['aws', 'gcp', 'azure', 'subdomain_takeover']

export default function VulnCloudPage() {
  ...
  useEffect(() => {
    setLoading(true)
    const qs = new URLSearchParams({ domain: 'cloud', ...(tool && { tool }), ...(severity && { severity }) })
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-6 space-y-4">
        <div>
          <h1 className="text-base font-semibold text-[#e2e8f0]">Cloud</h1>
          <p className="text-[#4a5568] text-xs mt-0.5">
            AWS · GCP · Azure · Subdomain Takeover
          </p>
        </div>

        <div className="grid grid-cols-4 gap-2">
          {[
            { label: 'AWS',     desc: 'S3 exposure · Metadata SSRF · CloudFront bypass' },
            { label: 'GCP',     desc: 'GCS public · Metadata SSRF' },
            { label: 'Azure',   desc: 'Blob public · Metadata SSRF' },
            { label: 'Takeover', desc: 'CNAME → unregistered cloud service' },
          ].map(c => (
            <div key={c.label} className="bg-[#0d1117] border border-[#0987a0] rounded p-3">
              <p className="text-[11px] font-semibold text-[#76e4f7]">{c.label}</p>
              <p className="text-[10px] text-[#4a5568] mt-0.5">{c.desc}</p>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-[#141720] border border-[#1e2330] rounded-lg p-8 text-center">
            <p className="text-[#4a5568] text-sm">Chưa có Cloud findings</p>
            <p className="text-[#2d3748] text-xs mt-1">
              Auto-detect cloud provider từ hostname (*.amazonaws.com, *.googleapis.com, *.azurewebsites.net)
            </p>
          </div>
        ) : (
